...
#      the engine's api/), GPL-3.0-only everywhere else. This rule covers
#      documentation too, so a GPL header cannot be copy-pasted into an
...
if [ "${#wrong_license[@]}" -gt 0 ]; then
    ...
    printf '  %s\n' "${wrong_license[@]}"
    echo
    echo "  Files under stubs/ and AGENTS.md are MIT so mod authors can vendor them into"
...
if [ "$status" -ne 0 ]; then
  ...
  (`--` for Lua, `#` for shell, `<!-- ... -->` for Markdown; MIT under stubs/.)
EOF
else
